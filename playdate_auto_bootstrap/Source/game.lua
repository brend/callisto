...
build_view = function()
    local l6 = { scene = { tag = "Splash" }, direction = { tag = "Still" }, side = { tag = "Left" } }
    local l7 = (function(__base) local __tmp = {}; for k, v in pairs(__base) do __tmp[k] = v end; __tmp.scene = read_scene(); return __tmp end)(l6)
...
update = function()
    ...
    playdate.graphics.drawText(ViewModel_heading(l9), 20, 70)
    playdate.graphics.drawText(scene_title(l9.scene), 20, 90)
    playdate.graphics.drawText(direction_label(l9.direction), 20, 110)
    playdate.graphics.drawText(side_label(l9.side), 20, 130)
    playdate.graphics.drawText(scene_hint(l9.scene), 20, 150)
    return nil
end
