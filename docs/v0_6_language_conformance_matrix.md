...
| Record field punning (initializers) | Covered | `src/main.rs` (`parser_accepts_trailing_commas_and_record_field_punning`, `record_field_punning_all_shorthand_compiles_and_codegen`) | `src/main.rs` (`parser_accepts_trailing_commas_and_record_field_punning`, `record_field_punning_all_shorthand_compiles_and_codegen`) | `src/main.rs` (`record_field_punning_all_shorthand_compiles_and_codegen`) | Mixed (`Point { x, y = base }`) and all-shorthand (`Pair { a, b }`) both covered. Added in `M2`. |
