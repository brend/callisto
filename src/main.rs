...
fn run() -> Result<(), i32> {
    ...
    match cli.command {
        ...
        Command::InitPlaydate { dir } => init_playdate_template_command(&dir),
        Command::Check {
            input,
            config,
            module_roots,
...
fn init_playdate_template_command(dir: &Path) -> Result<(), i32> {
...
mod tests {
    ...
    use super::{
        ...
        compile_pipeline_with_options, emit_lua_command, emit_lua_command_with_overrides,
        init_playdate_template_command, resolve_output_path, resolve_project_options,
    };
...
"#;
    ...
    fn init_playdate_template_creates_expected_files() {
        ...
        init_playdate_template_command(&root).expect("init playdate template");
    ...
    fn init_playdate_template_rejects_non_empty_dir() {
        ...
        assert_eq!(init_playdate_template_command(&root).unwrap_err(), 1);
